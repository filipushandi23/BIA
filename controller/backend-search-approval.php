<?php
	...
	if(isset($_POST['param'])){
		...
		$sql = "
			SELECT tw.security_code, tw.no_rek_pengirim,tw.no_rek_penerima, tw.jumlah_transfer,tw.status, 
		...
		if(mysqli_num_rows($result)) {
			while($row = mysqli_fetch_assoc($result)){
				...
				$myObj->no_rek_penerima = $row['no_rek_penerima'];
				$myObj->nama_penerima = $row['nama_penerima'];
